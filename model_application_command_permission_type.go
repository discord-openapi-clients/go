/*
Discord HTTP API (Preview)

Preview of the Discord v10 HTTP API specification. See https://discord.com/developers/docs for more details.

API version: 10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package discord

import (
	"encoding/json"
)

// checks if the ApplicationCommandPermissionType type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ApplicationCommandPermissionType{}

// ApplicationCommandPermissionType struct for ApplicationCommandPermissionType
type ApplicationCommandPermissionType struct {
}

// NewApplicationCommandPermissionType instantiates a new ApplicationCommandPermissionType object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewApplicationCommandPermissionType() *ApplicationCommandPermissionType {
	this := ApplicationCommandPermissionType{}
	return &this
}

// NewApplicationCommandPermissionTypeWithDefaults instantiates a new ApplicationCommandPermissionType object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewApplicationCommandPermissionTypeWithDefaults() *ApplicationCommandPermissionType {
	this := ApplicationCommandPermissionType{}
	return &this
}

func (o ApplicationCommandPermissionType) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ApplicationCommandPermissionType) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	return toSerialize, nil
}

type NullableApplicationCommandPermissionType struct {
	value *ApplicationCommandPermissionType
	isSet bool
}

func (v NullableApplicationCommandPermissionType) Get() *ApplicationCommandPermissionType {
	return v.value
}

func (v *NullableApplicationCommandPermissionType) Set(val *ApplicationCommandPermissionType) {
	v.value = val
	v.isSet = true
}

func (v NullableApplicationCommandPermissionType) IsSet() bool {
	return v.isSet
}

func (v *NullableApplicationCommandPermissionType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableApplicationCommandPermissionType(val *ApplicationCommandPermissionType) *NullableApplicationCommandPermissionType {
	return &NullableApplicationCommandPermissionType{value: val, isSet: true}
}

func (v NullableApplicationCommandPermissionType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableApplicationCommandPermissionType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


