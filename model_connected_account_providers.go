/*
Discord HTTP API (Preview)

Preview of the Discord v10 HTTP API specification. See https://discord.com/developers/docs for more details.

API version: 10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package discord

import (
	"encoding/json"
)

// checks if the ConnectedAccountProviders type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConnectedAccountProviders{}

// ConnectedAccountProviders struct for ConnectedAccountProviders
type ConnectedAccountProviders struct {
}

// NewConnectedAccountProviders instantiates a new ConnectedAccountProviders object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConnectedAccountProviders() *ConnectedAccountProviders {
	this := ConnectedAccountProviders{}
	return &this
}

// NewConnectedAccountProvidersWithDefaults instantiates a new ConnectedAccountProviders object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConnectedAccountProvidersWithDefaults() *ConnectedAccountProviders {
	this := ConnectedAccountProviders{}
	return &this
}

func (o ConnectedAccountProviders) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConnectedAccountProviders) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	return toSerialize, nil
}

type NullableConnectedAccountProviders struct {
	value *ConnectedAccountProviders
	isSet bool
}

func (v NullableConnectedAccountProviders) Get() *ConnectedAccountProviders {
	return v.value
}

func (v *NullableConnectedAccountProviders) Set(val *ConnectedAccountProviders) {
	v.value = val
	v.isSet = true
}

func (v NullableConnectedAccountProviders) IsSet() bool {
	return v.isSet
}

func (v *NullableConnectedAccountProviders) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConnectedAccountProviders(val *ConnectedAccountProviders) *NullableConnectedAccountProviders {
	return &NullableConnectedAccountProviders{value: val, isSet: true}
}

func (v NullableConnectedAccountProviders) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConnectedAccountProviders) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


