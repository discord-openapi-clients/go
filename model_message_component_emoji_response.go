/*
Discord HTTP API (Preview)

Preview of the Discord v10 HTTP API specification. See https://discord.com/developers/docs for more details.

API version: 10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the MessageComponentEmojiResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &MessageComponentEmojiResponse{}

// MessageComponentEmojiResponse struct for MessageComponentEmojiResponse
type MessageComponentEmojiResponse struct {
	Id *string `json:"id,omitempty" validate:"regexp=^(0|[1-9][0-9]*)$"`
	Name string `json:"name"`
	Animated NullableBool `json:"animated,omitempty"`
}

type _MessageComponentEmojiResponse MessageComponentEmojiResponse

// NewMessageComponentEmojiResponse instantiates a new MessageComponentEmojiResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMessageComponentEmojiResponse(name string) *MessageComponentEmojiResponse {
	this := MessageComponentEmojiResponse{}
	this.Name = name
	return &this
}

// NewMessageComponentEmojiResponseWithDefaults instantiates a new MessageComponentEmojiResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMessageComponentEmojiResponseWithDefaults() *MessageComponentEmojiResponse {
	this := MessageComponentEmojiResponse{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *MessageComponentEmojiResponse) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MessageComponentEmojiResponse) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *MessageComponentEmojiResponse) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *MessageComponentEmojiResponse) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value
func (o *MessageComponentEmojiResponse) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *MessageComponentEmojiResponse) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *MessageComponentEmojiResponse) SetName(v string) {
	o.Name = v
}

// GetAnimated returns the Animated field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MessageComponentEmojiResponse) GetAnimated() bool {
	if o == nil || IsNil(o.Animated.Get()) {
		var ret bool
		return ret
	}
	return *o.Animated.Get()
}

// GetAnimatedOk returns a tuple with the Animated field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MessageComponentEmojiResponse) GetAnimatedOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.Animated.Get(), o.Animated.IsSet()
}

// HasAnimated returns a boolean if a field has been set.
func (o *MessageComponentEmojiResponse) HasAnimated() bool {
	if o != nil && o.Animated.IsSet() {
		return true
	}

	return false
}

// SetAnimated gets a reference to the given NullableBool and assigns it to the Animated field.
func (o *MessageComponentEmojiResponse) SetAnimated(v bool) {
	o.Animated.Set(&v)
}
// SetAnimatedNil sets the value for Animated to be an explicit nil
func (o *MessageComponentEmojiResponse) SetAnimatedNil() {
	o.Animated.Set(nil)
}

// UnsetAnimated ensures that no value is present for Animated, not even an explicit nil
func (o *MessageComponentEmojiResponse) UnsetAnimated() {
	o.Animated.Unset()
}

func (o MessageComponentEmojiResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o MessageComponentEmojiResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	toSerialize["name"] = o.Name
	if o.Animated.IsSet() {
		toSerialize["animated"] = o.Animated.Get()
	}
	return toSerialize, nil
}

func (o *MessageComponentEmojiResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varMessageComponentEmojiResponse := _MessageComponentEmojiResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varMessageComponentEmojiResponse)

	if err != nil {
		return err
	}

	*o = MessageComponentEmojiResponse(varMessageComponentEmojiResponse)

	return err
}

type NullableMessageComponentEmojiResponse struct {
	value *MessageComponentEmojiResponse
	isSet bool
}

func (v NullableMessageComponentEmojiResponse) Get() *MessageComponentEmojiResponse {
	return v.value
}

func (v *NullableMessageComponentEmojiResponse) Set(val *MessageComponentEmojiResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableMessageComponentEmojiResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableMessageComponentEmojiResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMessageComponentEmojiResponse(val *MessageComponentEmojiResponse) *NullableMessageComponentEmojiResponse {
	return &NullableMessageComponentEmojiResponse{value: val, isSet: true}
}

func (v NullableMessageComponentEmojiResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMessageComponentEmojiResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


